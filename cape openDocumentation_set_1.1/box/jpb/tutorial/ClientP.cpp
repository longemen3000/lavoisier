// ********************************************************************
//
// Using the SMST Component: Partitioning Analysis on a Flowsheet
//
...
int main(int argc, char* const* argv) {
  ...
  try {
	...
	cout<<"         Is the Partitioning Analysis available ? : ";
	CapeBoolean reply=nserver->PartitioningImplementation();
	if(char(1)==reply) cout<<"Yes"<<endl;
	else cout<<"No"<<endl;

	//     Create the Partitioning Analysis
	cout<<endl;
	cout<<"  -->  Partitioning Analysis Creation"<<endl;
	ICapeNumericAnalysis_ptr num=nserver->CreateNumericAnalysis(PARTITIONING, "Partitioning Analysis");
	if(CORBA::is_nil(num)==char(1)) throw "CreateNumericAnalysis() - ObjRef=NIL";
	cout<<"         Does the Partitioning Analysis have specific Parameters ? :";
	if(char(0)==num->ParameterAvailable()) cout<<" No"<<endl;
	else cout<<" Yes"<<endl;

	//     Perform the Partitioning Analysis on the flowsheet
	cout<<endl;
	cout<<"  -->  Partitioning Analysis Computation "<<endl;
	num->Perform(graph);

	//     Display the Partitioning Analysis Results
	cout<<endl;
	cout<<"  -->  Partitioning Analysis Results"<<endl;
	long int partcount=(ICapeNumericPartitioning::_narrow(num))->GetPartitionCount();
	for(long int i=1;i<=partcount;i++){
		cout<<"          * Partition number "<<i<<endl;
		CapeLongSequence_var listofunit =(ICapeNumericPartitioning::_narrow(num))->GetUnitsInPartition(i);
